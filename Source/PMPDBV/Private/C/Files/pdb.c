...
static short GetAtom(Point3D *coordPtr, ATOM_TYPEp typePtr, char query[5], char atm_type[2], short n);
...
int AllocateMemoryForNewLayer(short layerL)
{
  ...
  PDB[layerL].GroupsHdl = malloc((long)kReadGroupChunk * sizeof(void *));
  if (PDB[layerL].GroupsHdl == NULL)
  {
    ShowMessage("Not enough memory for a new Layer", FALSE);
    return (1);
  }
  PDB[layerL].AtomNamesHdl = malloc((long)kReadGroupChunk * sizeof(void *));
...
{
  char s[256], s2[16];
  short err, k, rslt, nbSelfConnected;
...
static short GetAtom(Point3D *coordPtr, ATOM_TYPEp typePtr, char query[5], char atm_type[2], short n /*, unsigned short *Bfactor*/)
{
  short k = -1;
...
static short Read_aa(unsigned short i)
{
  ...
  AtomNamesHdl[i] = calloc((long)(aaName[index].nbPolarH + nbatms), sizeof(ATOM_TYPE));
...
static short Read_NT_Backbone(unsigned short i, long AGsize, long ATsize, GLYp *p, ATOM_TYPEp *AtTypePtr)
{
  ...
  GroupsHdl[i] = malloc(AGsize);
  if (GroupsHdl[i] == NULL)
    return (kOverflow);

  *p = GroupsHdl[i];

  AtomNamesHdl[i] = calloc(1, ATsize);
  if (AtomNamesHdl[i] == NULL)
    return (kOverflow);
